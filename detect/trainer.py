...
class IgniteEngine(Engine):
    ...
    def train(engine, mini_batch):
        ...
        context, response, target = mini_batch.context, mini_batch.response, mini_batch.target
        target = target.to(engine.device)

        context = context[:, :(engine.config.max_length//2)]
        ...
        loss =engine.crit(y_hat, target)
        loss.backward()

        if isinstance(target, torch.LongTensor) or isinstance(target, torch.cuda.LongTensor):
            accuracy = (torch.argmax(y_hat, dim=-1) == target).sum() / float(target.size(0))
        else:
            accuracy = 0
